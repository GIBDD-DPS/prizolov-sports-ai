...
print("=" * 50); print("🚀 PRIZOLOV SPORTS AI v9.65 STARTED (STABLE CORS MONOLITH)"); print(f"📅 UTC: {datetime.datetime.utcnow().isoformat()}"); print(f"🔍 PORT: {os.environ.get('PORT', '8080 (default)')}"); print("=" * 50); sys.stdout.flush()
...
app = FastAPI(title="Prizolov Sports AI - Monolith API", version="9.65")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def cors_handler_middleware(request: Request, call_next):
    if request.method == "OPTIONS":
        return JSONResponse(status_code=200, content="OK", headers={"Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, POST, OPTIONS", "Access-Control-Allow-Headers": "Content-Type, Accept, Authorization"})
    response = await call_next(request)
    response.headers["Access-Control-Allow-Origin"] = "*"
    return response
@app.get("/")
async def read_root_get(): return {"status": "ok", "message": "Prizolov Monolith Fixed Middleware Ready"}
@app.post("/api/state")
