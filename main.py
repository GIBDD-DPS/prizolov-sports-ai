...
print("=" * 50); print("🚀 PRIZOLOV SPORTS AI v9.80 STARTED (STABLE API RESPONSE)"); print(f"📅 UTC: {datetime.datetime.utcnow().isoformat()}"); print(f"🔍 PORT: {os.environ.get('PORT', '8080 (default)')}"); print("=" * 50); sys.stdout.flush()
...
for subdir in ["agent_bridge", "prizolov_sports_ai", "orchestrator", "core", "api", "modules"]:
    ...
    if full_path.exists() and str(full_path) not in sys.path: sys.path.insert(0, str(full_path))
    nested_path = current_dir / "prizolov_sports_ai" / subdir
    if nested_path.exists() and str(nested_path) not in sys.path: sys.path.insert(0, str(nested_path))
...
try:
    ...
    except ImportError: EventDiscoveryEngine, PrizolovSportsOrchestrator = None, None
class FallbackDiscovery:
    def __init__(self) -> None: self._events = []
    async def start_auto_discovery(self) -> None: await self._fetch()
    def stop(self) -> None: pass
    async def _fetch(self) -> None:
        now = datetime.datetime.utcnow(); self._events = []
        for sport, cfg in SPORTS_CONFIG.items():
            leagues, teams = cfg.get("leagues", []), cfg.get("teams", [])
            if not leagues or len(teams) < 2: continue
            for i in range(2):
                home_team = random.choice(teams); available_away = [t for t in teams if t != home_team]
                if not available_away: continue
                self._events.append({"match_id": f"{sport}_{i}_{random.randint(100, 999)}", "sport": sport, "league": random.choice(leagues), "home_team": home_team, "away_team": random.choice(available_away), "start_time": now.isoformat(), "status": "live", "betting_interest": 0.85})
...
class MockOrchestrator:
    def __init__(self, discovery_engine): self.discovery_engine, self.line_cache = discovery_engine, {}
...
async def scan_loop_task():
    ...
    try:
        if orchestrator_instance: await orchestrator_instance.run_initial_analysis()
    except Exception as e: logger.error(f"❌ Ошибка первичного анализа оркестратора: {e}")
...
async def startup_event():
    ...
    discovery_instance = EventDiscoveryEngine(refresh_interval=45) if EventDiscoveryEngine else FallbackDiscovery()
    await discovery_instance.start_auto_discovery()
    if PrizolovSportsOrchestrator:
        try: orchestrator_instance = PrizolovSportsOrchestrator(target_agent_host="localhost:50051", mock_mode=True, discovery_engine=discovery_instance)
        except Exception as e: orchestrator_instance = MockOrchestrator(discovery_engine=discovery_instance)
    else: orchestrator_instance = MockOrchestrator(discovery_engine=discovery_instance)
...
@app.middleware("http")
async def hard_cors_injector_middleware(request: Request, call_next):
    if request.method == "OPTIONS":
        return JSONResponse(status_code=200, content="OK", headers={"Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, POST, OPTIONS", "Access-Control-Allow-Headers": "Content-Type, Accept, Authorization, X-Requested-With"})
    ...
    return response
@app.get("/")
async def read_root_get(): return {"status": "ok", "message": "Prizolov Monolith Stable Headers Ready"}
@app.post("/api/state")
async def read_root(request: Request):
    ...
    for i in range(5):
        ...
        recommendations.append({"league": random.choice(["РПЛ", "КХЛ", "АПЛ"]), "sport": random.choice(["football", "hockey"]), "home": t_home, "away": t_away, "line": random.choice(["П1", "Х", "ТБ (2.5)"]), "probability": round(random.uniform(0.65, 0.92), 2), "confidence": random.choice(["high", "med"]), "coefficient": round(random.uniform(1.45, 3.10), 2)})
    try:
    ...
    return JSONResponse(status_code=200, content={"status": "ok", "timestamp": datetime.datetime.utcnow().isoformat(), "match_info": match_info, "recommendations": recommendations}, headers={"Access-Control-Allow-Origin": "*"})
