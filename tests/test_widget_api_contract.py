...
class TestWidgetApiContract(unittest.TestCase):
    ...
    def test_value_only_premium_mode_filters_lines(self):
        response = self.client.get(
            "/api/all-events?lang=en&limit=5&premium_only=true&premium_min_edge=-0.2&premium_min_bookmakers_support=1"
        )
        self.assertEqual(response.status_code, 200)

        payload = response.json()
        self.assertIn("filters", payload)
        self.assertIn("value_only", payload["filters"])
        self.assertTrue(payload["filters"]["value_only"]["effective"]["premium_only"])
        self.assertIn("readiness_flags", payload)
        self.assertTrue(payload["readiness_flags"].get("value_only_premium_lines", False))

        effective_filters = payload["filters"]["value_only"]["effective"]
        min_edge = float(effective_filters["min_edge"])
        min_support = int(effective_filters["min_bookmakers_support"])
    ...
    def test_state_and_widget_support_value_only_flags(self):
        state_response = self.client.post(
            "/api/state?premium_only=true&premium_min_edge=-0.2&premium_min_bookmakers_support=1"
        )
        self.assertEqual(state_response.status_code, 200)
        state_payload = state_response.json()
        self.assertIn("filters", state_payload)
        self.assertIn("value_only", state_payload["filters"])
        self.assertTrue(state_payload["filters"]["value_only"]["effective"]["premium_only"])
        self.assertIn("readiness_flags", state_payload)
        self.assertTrue(state_payload["readiness_flags"].get("value_only_premium_lines", False))
        ...
        widget_response = self.client.post(
            ...
            json={
                ...
                "lang": "en",
                "premium_only": True,
                "premium_min_edge": -0.2,
                "premium_min_bookmakers_support": 1,
            },
        )
        self.assertEqual(widget_response.status_code, 200)
        widget_payload = widget_response.json()
        self.assertIn("filters", widget_payload)
        self.assertIn("value_only", widget_payload["filters"])
        self.assertIn("readiness_flags", widget_payload)
        for event in widget_payload.get("events", []):
            for rec in event.get("recommendations", []):
                self.assertTrue(rec.get("is_premium", False))
