...
# Version: 5.03 (Minor Refactor & WP/Elementor Prep)
#
# CHANGELOG (5.02 → 5.03):
# - Уточнены типы и docstring-и
# - Упорядочены импорты
# - Лёгкая чистка структуры без изменения логики
# - Подготовка структуры под API-вывод статуса дедупликации для WordPress Elementor
#
...
class NetworkTrafficCompressor:
    ...
    def __init__(self, odds_epsilon: float = 0.02, coord_epsilon_metrics: float = 0.15) -> None:
        """
        Args:
            odds_epsilon: чувствительность к изменению коэффициентов.
            coord_epsilon_metrics: чувствительность к изменению координат мяча.
        """
        self.odds_eps = odds_epsilon
        self.coord_eps = coord_epsilon_metrics
        self.last_sent_package: Optional[Dict[str, Any]] = None

    def _is_odds_diff_negligible(
        self,
        ...
        Returns:
            True — если изменения незначительны.
        """
    ...
    def should_skip_frame(self, current_package: Dict[str, Any]) -> bool:
        ...
        Args:
            current_package: текущий пакет данных (ball_state, line_data).

        Returns:
            True — если фрейм можно пропустить (ничего не изменилось).
        """
        ...
        dist = math.sqrt(
            (old_ball.get("x", 0.0) - new_ball.get("x", 0.0)) ** 2 +
        ...
        for market_key in ["main_outcomes", "totals", "handicaps"]:
            if not self._is_odds_diff_negligible(
                old_line.get(market_key, []),
                new_line.get(market_key, [])
            ):
                self.last_sent_package = current_package
                return False
    ...
    def decompress_payload(self, compressed_bytes: bytes) -> str:
        ...
        Args:
            compressed_bytes: бинарный буфер.
