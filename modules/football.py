...
class FootballAnalyticsModule:
    ...
    def calculate_homography_coords(self, pixel_coords: Tuple[float, float], h_matrix: np.ndarray) -> Tuple[float, float]:
        if h_matrix is None: return pixel_coords
        point = np.array([pixel_coords, pixel_coords, 1.0])
        transformed = np.dot(h_matrix, point)
        if transformed == 0: return pixel_coords
        return (float(transformed / transformed), float(transformed / transformed))

    def evaluate_offside(self, defenders_metric: List[float], attackers_metric: List[float], ball_metric_x: float, is_pass_event: bool) -> Tuple[bool, List[int]]:
        if not is_pass_event or len(defenders_metric) < 2: return False, []
    ...
    def process_frame_data(self, tracking_data: Dict[str, Any], game_time_str: str, time_left_ratio: float) -> Dict[str, Any]:
        ...
        if ball_owner_team == "A": self.possession_frames_a += 1
        elif ball_owner_team == "B": self.possession_frames_b += 1

        total_p_frames = self.possession_frames_a + self.possession_frames_b
        p_pct_a = 50.0
        if total_p_frames > 0: p_pct_a = round((self.possession_frames_a / total_p_frames) * 100, 1)
        ...
        base_markets = self.lg.calculate_poisson_line(live_lambda_a, live_lambda_b, time_left_ratio, self.current_score_a, self.current_score_b)
        corners_markets = self.lg.calculate_poisson_line(5.0 * (live_lambda_a / max(self.base_lambda_a, 0.1)), 4.5 * (live_lambda_b / max(self.base_lambda_b, 0.1)), time_left_ratio, self.corners_a, self.corners_b)
        ...
        return {
            "match_id": self.match_id, "sport": "football", "game_time": game_time_str,
            "ball_state": {"x": tracking_data.get("ball_x", 0.0), "y": tracking_data.get("ball_y", 0.0), "vx": tracking_data.get("ball_vx", 0.0), "vy": tracking_data.get("ball_vy", 0.0), "last_touch_id": tracking_data.get("last_touch_id", -1)},
            "metrics": {"possession_a": p_pct_a, "possession_b": p_pct_b, "xg_a": tracking_data.get("live_xg_a", 0.0), "xg_b": tracking_data.get("live_xg_b", 0.0), "dangerous_attacks_a": danger_attacks_a, "dangerous_attacks_b": danger_attacks_b},
