...
class BasketballAnalyticsModule:
    ...
    def analyze_ball_trajectory_3d(self, 
        ...
        Использует размер bounding box мяча: чем меньше коробка детекции, тем выше мяч улетает вверх 
        (или дальше от камеры к кольцу) по законам перспективы.
        """
        if ball_bounding_box_size <= 0:
            return 0.0
        # Инверсная зависимость размера на экране от реального удаления/высоты объекта
    ...
    def process_frame_data(self, 
        ...
        """
        Главный цикл обработки баскетбольного кадра. Оценивает Live Pace,
        детектирует типы бросков и пересчитывает высокорезультативную широкую линию.
        """
        self.seconds_elapsed = max(elapsed_seconds, 1)
        
        # 1. Динамический расчет Live Pace матча
        ...
        if detected_possessions > 0:
            ...
            # Экстраполируем текущую скорость игры на полный 40-минутный матч (2400 секунд)
            live_pace = (self.total_possessions_observed / self.seconds_elapsed) * 2400.0
            # Ограничиваем адекватные коридоры темпа для защиты от шумов
            live_pace = max(min(live_pace, 110.0), 60.0)
        else:
            live_pace = self.base_pace

        # 2. Корректировка атакующей эффективности на основе Live xShot метрик
        # xShot - вероятность попадания конкретного броска по оценке ИИ
        xshot_a = tracking_data.get("recent_xshot_avg_a", 0.45)
        xshot_b = tracking_data.get("recent_xshot_avg_b", 0.43)
        
        # 3. Адаптация ORTG (эффективности на владение)
        live_eff_a = self.base_efficiency_a * (xshot_a / 0.45)
        live_eff_b = self.base_efficiency_b * (xshot_b / 0.43)

        # 4. Флаг заморозки линии (Бонусная ситуация: штрафные броски или челлендж тренера)
        ...
        # При критическом переборе фолов (больше 4) ценность владений растет из-за штрафных
        if self.fouls_a > 4: live_eff_b += 0.08
        if self.fouls_b > 4: live_eff_a += 0.08

        # 5. Расчет баскетбольной линии через нормальную аппроксимацию ядра
        ...
        # 6. Дополнительные рынки широкой линии: Тотал командных фолов за период/матч
        # Используем Пуассон для фолов, так как это редкие дискретные события
        ...
        # Применяем статус заморозки к основным исходам
        if is_suspended:
            for m in base_markets["main_outcomes"]: m["is_suspended"] = True
            for t in base_markets["totals"]: t["is_suspended"] = True

        # Рассчитываем Z координату из кадра для передачи метаданных
        ...
        # Сборка финального баскетбольного JSON-пакета
        output_package = {
            ...
            "ball_state": {
                "x": raw_bx,
                "y": raw_by,
                "vx": bz, # Передаем рассчитанную Z-высоту в качестве вектора для Агента
            ...
            "metrics": {
                ...
                "possession_b": 0.0,
                "xg_a": xshot_a, # Для баскетбола xG эквивалентен xShot
                "xg_b": xshot_b,
                "dangerous_attacks_a": tracking_data.get("paint_entries_a", 0), # Проходы в трехсекундную зону
