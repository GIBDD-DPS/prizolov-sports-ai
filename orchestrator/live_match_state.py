...
# Безопасный импорт модулей ядра с созданием заглушек в случае их отсутствия
try:
    from core.radar_renderer import SportsRadarRenderer
except ImportError:
    class SportsRadarRenderer:
        def __init__(self, sport): pass
...
try:
    ...
    class RefereeSeverityAnalytics:
        ...
        def calibrate_discipline_intensity(self, val): return 0.25
...
class LiveMatchState:
    ...
    Полностью защищен от падений и блокировок портов Amvera.
    """

    def __init__(self, match_id: str):
        self.match_id = match_id

        # Безопасная инициализация движков
    ...
    def update_score(self, score: str): self.score = score
    def update_time(self, seconds: int): self.time_seconds = seconds
    def update_ball(self, x: float, y: float): self.ball_state = {"x": x, "y": y}
    def update_players(self, team_a: list, team_b: list):
        self.team_a_players = team_a
        self.team_b_players = team_b

    def build_state(self) -> Dict[str, Any]:
        """Собирает полный live-state матча, устойчивый к любым внутренним сбоям модулей."""
